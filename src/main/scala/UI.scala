            ...
            aside(
              ...
              div(
                ...
                ol(
                  ...
                  ranking("01", "Context retrieval", "Retrieve phrase, topic, prompt, discourse, and English fallback candidates."),
                  ranking("02", "Structured branches", "Choose among labeled subtrees containing direct children and sample leaves."),
                  ranking("03", "Beam retention", "Keep the three branches with the strongest root probabilities."),
                  ranking("04", "Leaf commitment", "Choose up to 255 leaves by combined root and leaf probability."),
                ),
                p(
                  `class` := "mt-4 border-t border-white/10 pt-4 text-[11px] leading-5 text-slate-500",
                  s"The host ranks ${RankedWordSource.EnglishDictionarySize} packaged English words, induces a ${RankedWordSource.MaxTreeCandidates}-candidate tree, retains K=${LexicalTree.BeamWidth} branches, and presents no more than 255 leaves.",
                ),
              ),
            ),
          ),
